...
export default function public_good() {
  ...
  return {
    id: "public_good",
    title: "Public Good",
    description: "Each player receives 10 points. Between 0 and 10 points can be put towards a shared pot. The pot is doubled and the points are distributed back evenly to all players, except for the player making the smallest contribution, who receives nothing. Every player keeps the points they don't contribute.",
    ...
    onMessage: (ctx: Game, type: string, playerId: string, data: any): null | { scores: Record<string, number>; data: any } => {
      ...
      if (contributions.size === ctx.players.size) {
        let inOrder = [...contributions].map(([playerId, contribution]) => { return {playerId, contribution}; });
        let total = inOrder.reduce((acc, {contribution}) => acc + contribution, 0);
