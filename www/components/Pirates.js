...
function Vote({ game }) {
  ...
  return html`
    <div>
      <${Button} onClick=${() => submit(true)} title="Agree" disabled=${game.player.ready} style=${styles.voteButton}/>
      <${Button} onClick=${() => submit(false)} title="Reject" disabled=${game.player.ready} style=${styles.voteButton}/>
    </div>
  `;
}
...
function ProposedSplit({ captain, alive, split, noTitle }) {
  return html`
    ${noTitle === true ? null : html`<p style=${styles.text}>${captain} proposed the following split:</p>`}
    ${alive.map(player => {
      return html`
        <div style=${styles.splitRow}>
          <${PlayerChip} ...${player} />
...
function Play({ game }) {
  ...
  const findPlayer = playerId => game.players.find(({id}) => id === playerId);
  const playersAlive = game.round.data.playersAlive.map(id => findPlayer(id));
  const playersDead = game.players.filter(({id}) => !game.round.data.playersAlive.includes(id));

  const isCaptain = game.round.data.captain === game.player.id;
  const captain = isCaptain ? "You" : html`<${PlayerChip} ...${findPlayer(game.round.data.captain)} />`;
  const propsedSplit = game.round.data.split;

  const isDead = !game.round.data.playersAlive.includes(game.player.id);

  return html`
    ${ propsedSplit == null && html`<p style=${styles.text}>${captain} ${isCaptain ? "are" : "is"} captain.</p>`}
    ${
      isCaptain &&
      propsedSplit == null &&
    ...
    ${propsedSplit != null && !isCaptain && !isDead && html`<${Vote} game=${game} />`}
    ${
      game.round.data.lastVotes != null &&
      html`<${VoteResults} votes=${game.round.data.lastVotes} players=${game.players} />`
    }
